...
const getAllResources = async (request, response, next) => {
    try {
        if(200) {
            ...
            response.status(200).json({success: {message: Found all resources},
            data: resources,
        statusCode: 200,
    }))
    catch (error) {
...
const createResource = async (request, response, next) => {
    ...
    const newResource = newResource({id, name, state, location, phone, website, services});
...
const editResource = async (request, response, next) => {
    ...
    try {Resource.findByIdAndUpdate(id, {$set: {
        name, 
        state,
        location, 
        phone, 
...
module.exports = {getAllResources, getResource, createResource, editResource, deleteResource};